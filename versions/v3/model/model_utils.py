...
import models
from flylora_dual import replace_with_dual_flylora
...
def process_query_tokens(args, tokenizer):
    ...
    if args.query_same:
        ...
        if args.checkpoint_dir is None:
            tokenizer.add_special_tokens({'additional_special_tokens': special_tokens + alignment_tokens})
...
def load_pretrained_model(args, model_name, config, kwargs):
    model_map = {'llama': models.llama_rec}
    ...
    for name, _ in model.named_modules():
        if 'layers.' in name and 'small_model' not in name:
            ...
            if layer_idx in target_layer_indices:
                if name.split('.')[-1] in target_module_types:
                    target_modules.append(name)
    return target_modules


def set_dual_flylora(args, model):
    target_modules = _collect_target_linear_modules(args, model)
    output_mix = args.flylora_output_mix
    if output_mix is None:
        output_mix = args.fusion_alpha
    replaced = replace_with_dual_flylora(
        model,
        target_modules,
        r=args.flylora_r,
        k=args.flylora_k,
    ...
    args.logger.info(f'Dual FlyLoRA injected modules: {replaced}; output_mix={output_mix}')

    for _, param in model.named_parameters():
        param.requires_grad = False

    need_train = ['flylora_B_text', 'flylora_B_id', 'score', 'norm', 'new_emb', 'small_model']
...
def freeze_model_layers(args, model):
    ...
    need_train = ['flylora_B_text', 'flylora_B_id', 'norm']
...
def apply_model_customizations(args, model):
    if args.use_flylora_dual:
        model = set_dual_flylora(args, model)
